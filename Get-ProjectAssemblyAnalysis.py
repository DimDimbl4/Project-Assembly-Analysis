...
class Config:
    ...
    @property
    def PROCMON_FILTERS(self) -> List[str]:
        return self.config["procmon_filters"]
...
class ProcMonTool(MonitoringTool):
    def start(self, target_dir: str, output_file: str) -> None:
        try:
            filter_file = "procmon_filter.pmc"
            with open(filter_file, "w") as f:
                f.write("\n".join(self.config.PROCMON_FILTERS))
            ...
            self.process = subprocess.Popen([
                ...
                "/LoadConfig", filter_file
            ], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            logging.info(f"ProcMon started. Logging to {output_file}")
        except Exception as e:
            logging.error(f"Failed to start ProcMon: {e}")
            raise
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            logging.error(f"Failed to terminate ProcMon: {e}")
...
class MonitoringManager:
    ...
    def start_monitoring(
        ...
        tool_classes = {
            (PlatformType.WINDOWS, MonitoringToolType.PROCMON): ProcMonTool,
...
class LogParser:
    ...
    def parse_procmon_csv(self, csv_path: str) -> Set[str]:
        used_files = set()
        try:
            ...
            logging.error(f"ProcMon CSV parsing failed: {e}")
        return used_files
...
class FileUsageAnalyzer:
    ...
    def analyze_unused_files(
        ...
        # Парсинг лога
        parser_methods = {
            MonitoringToolType.PROCMON: self.log_parser.parse_procmon_csv,
            MonitoringToolType.INOTIFYWAIT: self.log_parser.parse_inotifywait_log,
            MonitoringToolType.STRACE: self.log_parser.parse_strace_log,
            MonitoringToolType.AUDITD: self.log_parser.parse_auditd_log
        }
...
def main():
    ...
    try:
        ...
        analyzer = FileUsageAnalyzer(config)
